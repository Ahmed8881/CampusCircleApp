...
class AdminDashboardCourseAdapter(
    ...
    class AdminCourseViewHolder(itemView: View) : RecyclerView.ViewHolder(itemView) {
        ...
        val attendanceBar: ProgressBar = itemView.findViewById(R.id.adminCourseAttendanceBar)
    }
    ...
    override fun onBindViewHolder(holder: AdminCourseViewHolder, position: Int) {
        ...
        holder.courseAttendance.text = "${item.attendance}% attendance"
        holder.courseEnrollments.text = "${item.enrollments} enrolled"
        holder.attendanceBar.progress = item.attendance.coerceIn(0, 100)
    }
